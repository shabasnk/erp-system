...
// // last used
// // this used for the fronend change for the shop owner based rprtmngmnt
...
export const createCustomer = async (req, res) => {
  try {
    // Extract shopId from authenticated user (added this line)
    ...
    if (cleanedPhone.length !== 12) {
      return res.status(400).json({
        success: false,
        message: 'Phone number must be exactly 12 digits (including country code)'
      });
    }
