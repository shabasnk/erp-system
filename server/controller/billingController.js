// // for after slving the invoice creation issue after the shop owner based report
...
//       // ✅ Add this validation (like in customerController)
...
//     // Extract shopId from authenticated user (from token verification middleware)
...
export const searchProducts = async (req, res) => {
  try {
    ...
    // ✅ Add authentication check (like in createInvoice)
    if (!req.user || !req.user.shopId) {
      return res.status(401).json({
        success: false,
        message: 'Authentication required',
    ...
    const products = await Product.findAll({
      where: {
        [Op.and]: [
          ...
          {
            shopId: shopId, // Filter by the authenticated user's shopId
          },
          {
            isActive: true,
          }
      ...
      limit: 10,
      attributes: ["id", "name", "price", "description", "discountPrice", "stockQuantity"],
    });
